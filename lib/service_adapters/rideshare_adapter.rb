    ...
    def create_rideshare_query from, to, trip_datetime
      ...
      [
        ...
        ['orig', from_geo, from],
          ...
          query["#{p}.city"] = s1['locality']['short_name']
          query["#{p}.country"] = s1['country']['short_name']
          query["#{p}.county"] = s1['administrative_area_level_2']['short_name']
          query["#{p}.geocodeType"] = 'Address'
          query["#{p}.latLon.x"] = s2.lon
          query["#{p}.latLon.y"] = s2.lat
        # query["#{p}.line2"] = to.address2
        query["#{p}.postalCode"] = s1['postal_code']['short_name']
        query["#{p}.state"] = s1['administrative_area_level_1']['short_name']
        query["#{p}.street"] = s1['street_number']['short_name'] + ' ' + s1['route']['short_name']
        # query['dest.suburb'] = 
      end
      query['search'] = 'search'
      query['window'] = 3

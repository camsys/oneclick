...
/*
-- "v_Satisfaction_Surveys"

CREATE OR REPLACE VIEW public."v_Satisfaction_Surveys" AS 
 SELECT satisfaction_surveys.id AS survey_id,
    date_trunc('second'::text, satisfaction_surveys.updated_at) AS survey_datetime,
    satisfaction_surveys.trip_id AS survey_trip_id,
    satisfaction_surveys.satisfied AS survey_satisfied,
    satisfaction_surveys.comment AS survey_comment,
    btrim(regexp_replace(replace(satisfaction_surveys.reasoning, '-'::text, ''::text), '[\n\r'']'::text, ';'::text, 'g'::text), '; '::text) AS survey_reason
...
CREATE MATERIALIZED VIEW "v_Service_Accommodations" AS 
  ...
  FROM crosstab ('SELECT service_accommodations.service_id, accommodations.name, true As value FROM public.service_accommodations, public.accommodations WHERE accommodations.id = service_accommodations.accommodation_id ORDER BY 1,2'::text, 
                 $$VALUES ('curb_to_curb_name'),('door_to_door_name'),('driver_assistance_name'),('folding_wheelchair_accessible_name'),('motorized_wheelchair_accessible_name'),('wheelchair_lift_equipped_name') $$) 
  ct ( service_id integer,_curb_to_curb_name boolean,_door_to_door_name boolean,_driver_assistance_name boolean,_folding_wheelchair_accessible_name boolean,_motorized_wheelchair_accessible_name boolean,_wheelchair_lift_equipped_name boolean )
WITH DATA;
...
SELECT 
    ...
    ct._age_name AS service_age,
    ct._date_of_birth_name AS service_date_of_birth,
    ct._physically_disabled_name AS service_physically_disabled,
    ct._use_other_trans_name AS service_use_other_trans,
    ct._veteran_name AS service_veteran
  FROM crosstab ('SELECT service_characteristics.service_id, characteristics.name, service_characteristics.value FROM public.service_characteristics, public.characteristics WHERE characteristics.id = service_characteristics.characteristic_id ORDER BY 1,2'::text, 
                  $$VALUES ('ada_eligible_name'),('age_name'),('date_of_birth_name'),('physically_disabled_name'),('use_other_trans_name'),('veteran_name') $$) 
  ct ( service_id integer,_ada_eligible_name text,_age_name text,_date_of_birth_name text,_physically_disabled_name text,_use_other_trans_name text,_veteran_name text )
WITH DATA;
...
CREATE MATERIALIZED VIEW "v_Service_Trip_Purposes" AS 
  SELECT ct.service_id AS stp_service_id,
    ct._cancer_treatment_name AS service_cancer_treatment,
    ct._general_medical_name AS service_general_medical,
    ct._grocery_name AS service_grocery,
    ct._other_name AS service_other,
    ct._visit_senior_center_name AS service_visit_senior_center
  FROM crosstab ('SELECT service_trip_purpose_maps.service_id, trip_purposes.name, true as value FROM public.service_trip_purpose_maps, public.trip_purposes WHERE trip_purposes.id = service_trip_purpose_maps.trip_purpose_id ORDER BY 1,2'::text, 
                 $$VALUES ('cancer_treatment_name'),('general_medical_name'),('grocery_name'),('other_name'),('visit_senior_center_name') $$) 
  ct ( service_id integer,_cancer_treatment_name boolean,_general_medical_name boolean,_grocery_name boolean,_other_name boolean,_visit_senior_center_name boolean )
WITH DATA;
...
CREATE MATERIALIZED VIEW "v_Users_Accommodations" AS 
  SELECT ct.user_profile_id AS ua_user_id,
    ct._curb_to_curb_name AS user_curb_to_curb,
    ct._door_to_door_name AS user_door_to_door,
    ct._driver_assistance_name AS user_driver_assistance,
    ct._folding_wheelchair_accessible_name AS user_folding_wheelchair_accessible,
    ct._motorized_wheelchair_accessible_name AS user_motorized_wheelchair_accessible,
    ct._wheelchair_lift_equipped_name AS user_wheelchair_lift_equipped
  FROM crosstab ('SELECT user_accommodations.user_profile_id, accommodations.name, user_accommodations.value FROM public.accommodations, public.user_accommodations WHERE accommodations.id = user_accommodations.accommodation_id ORDER BY 1,2'::text, 
                 $$VALUES ('curb_to_curb_name'),('door_to_door_name'),('driver_assistance_name'),('folding_wheelchair_accessible_name'),('motorized_wheelchair_accessible_name'),('wheelchair_lift_equipped_name') $$) 
  ct ( user_profile_id integer,_curb_to_curb_name text,_door_to_door_name text,_driver_assistance_name text,_folding_wheelchair_accessible_name text,_motorized_wheelchair_accessible_name text,_wheelchair_lift_equipped_name text )
WITH DATA;
...
CREATE MATERIALIZED VIEW "v_Users_Characteristics" AS 
  SELECT ct.user_profile_id AS uc_user_id,
    ct._ada_eligible_name AS user_ada_eligible,
    ct._age_name AS user_age,
    ct._date_of_birth_name AS user_date_of_birth,
    ct._physically_disabled_name AS user_physically_disabled,
    ct._use_other_trans_name AS user_use_other_trans,
    ct._veteran_name AS user_veteran
  FROM crosstab ('SELECT user_characteristics.user_profile_id, characteristics.name, user_characteristics.value FROM public.characteristics, public.user_characteristics WHERE characteristics.id = user_characteristics.characteristic_id ORDER BY 1,2'::text, 
                 $$VALUES ('ada_eligible_name'),('age_name'),('date_of_birth_name'),('physically_disabled_name'),('use_other_trans_name'),('veteran_name') $$) 
  ct ( user_profile_id integer,_ada_eligible_name text,_age_name text,_date_of_birth_name text,_physically_disabled_name text,_use_other_trans_name text,_veteran_name text )
WITH DATA;
...
CREATE MATERIALIZED VIEW "v_Users_Desired_Modes" AS 
  SELECT ct.user_id AS udm_user_id,
    ...
    ct._mode_walk_name AS user_desired_mode_walk
...
/*CREATE OR REPLACE VIEW public."v_Trip_Surveys" AS 
 SELECT "v_Satisfaction_Surveys".*,
    "v_Trips".*,
    "v_Users".*
   FROM "v_Satisfaction_Surveys",
    "v_Trips",
    "v_Users"
  WHERE "v_Trips".trip_id = "v_Satisfaction_Surveys".survey_trip_id AND "v_Users".user_id = "v_Trips".trip_user_id;
*/

...
CREATE MATERIALIZED VIEW "v_Service_Accommodations" AS 
  SELECT ct.service_id AS sa_service_id,
    ...
    ct._stretcher_name AS service_stretcher_name
  FROM crosstab ( 'SELECT service_accommodations.service_id, accommodations.name, true As value FROM public.service_accommodations, public.accommodations WHERE accommodations.id = service_accommodations.accommodation_id ORDER BY 1,2'::text, $$VALUES ('curb_to_curb_name'),('door_through_door_name'),('folding_wheelchair_accessible_name'),('motorized_wheelchair_accessible_name'),('stretcher_name') $$)
ct (service_id integer,
...
SELECT ct.service_id AS sc_service_id,
    ...
    ct._medicaid_name AS service_medicaid
...
CREATE MATERIALIZED VIEW "v_Service_Trip_Purposes" AS 
  SELECT ct.service_id AS stp_service_id,
    ct._grocery_name AS service_cancer_treatment,
    ct._medical_name AS service_general_medical,
    ct._work_name AS service_general_purpose,
    ct._other_name AS service_grocery
...
CREATE MATERIALIZED VIEW "v_Users_Accommodations" AS 
  SELECT ct.user_profile_id AS ua_user_id,
    ...
    ct._stretcher_name AS user_wheelchair_lift_equipped
...
CREATE MATERIALIZED VIEW "v_Users_Characteristics" AS 
  SELECT ct.user_profile_id AS uc_user_id,
    ...
    ct._matp_name AS user_matp,
    ct._ada_eligible_name AS user_ada_eligible,
    ct._veteran_name AS user_veteran,
    ct._walk_distance_name AS user_walk_distance_dependent,
    ct._medicaid_name AS user_medicaid_dependent

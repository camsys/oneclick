...
CREATE MATERIALIZED VIEW "v_Trip_Places_From" AS 
  SELECT trip_places.trip_id,
    ...
    CASE WHEN (trip_places.raw_address IS NULL)
      THEN (trip_places.address1 || ' ' || trip_places.city || ' ' || trip_places.state || ' ' || trip_places.zip)
      ELSE trip_places.raw_address
    END AS "From_Raw_Address",

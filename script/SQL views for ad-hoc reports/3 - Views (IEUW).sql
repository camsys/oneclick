...
DROP VIEW IF EXISTS "v_Trip_Surveys";
...
/* Stage 3: Generic */
DROP VIEW IF EXISTS "v_Satisfaction_Surveys";
...
-- View: "v_Satisfaction_Surveys"

CREATE OR REPLACE VIEW "v_Satisfaction_Surveys" AS 
 SELECT satisfaction_surveys.id,
    date_trunc('second', satisfaction_surveys.updated_at) As "Survey_DateTime",
    satisfaction_surveys.trip_id,
    satisfaction_surveys.satisfied,
    satisfaction_surveys.comment,
    trim(regexp_replace(replace(reasoning, '-', ''), E'[\\n\\r'']', ';', 'g'), '; ') As reason
...
CREATE MATERIALIZED VIEW "v_Service_Trip_Purposes" AS 
  SELECT ct.service_id AS "STP_ID",
    ...
    ct._visit_senior_center_name AS "service_visit_senior_center_name"
...
CREATE MATERIALIZED VIEW "v_Users_Characteristics" AS 
  SELECT ct.user_profile_id AS "UC_ID",
    ...
    ct._use_other_trans_name AS "user_use_other_trans_name",
...
CREATE MATERIALIZED VIEW "v_Users_Desired_Modes" AS 
  ...
  FROM crosstab ('SELECT user_mode_preferences.user_id, modes.name, (user_mode_preferences.mode_id > 0) As "Mode_Desired" FROM public.modes, public.user_mode_preferences WHERE modes.id = user_mode_preferences.mode_id AND modes.visible = true ORDER BY 1,2'::text, 
                 $$VALUES ('mode_bus_name'),('mode_paratransit_name'),('mode_rail_name'),('mode_taxi_name'),('mode_transit_name') $$) 
  ct ( user_id integer,_mode_bus_name boolean,_mode_paratransit_name boolean,_mode_rail_name boolean,_mode_taxi_name boolean,_mode_transit_name boolean )
WITH DATA;
...
CREATE OR REPLACE VIEW "v_Trip_Surveys" AS
  SELECT 
    "v_Satisfaction_Surveys".id, 
    "v_Satisfaction_Surveys"."Survey_DateTime", 
    "v_Satisfaction_Surveys".satisfied, 
    "v_Satisfaction_Surveys".comment, 
    "v_Satisfaction_Surveys".reason,
    "v_Trips".*, 
    "v_Users".*
  FROM 
    public."v_Satisfaction_Surveys", 
    public."v_Trips", 
    public."v_Users"
  WHERE 
    "v_Trips"."Trip_ID" = "v_Satisfaction_Surveys".trip_id AND
    "v_Users"."User_ID" = "v_Trips".user_id;

...
function TripReviewPageRenderer(intervalStep, barHeight, tripResponse, filterConfigs, localeDictFinder, isItineraryUnselectable) {
    ...
    function addTripHtml(trip, inlineHelperKey) {
        ...
        if ($('#' + tripPartDivId).length === 0) { //new trip part
            ...
            trip.is_return_trip ? (header = localeDictFinder['return']) : (header = localeDictFinder['outbound']);
    ...
    function getCostFilterHtml(minCost, maxCost) {
        ...
        if (typeof(maxCost) === 'number' && typeof(minCost) === 'number' && maxCost > minCost) {
            ...
            tags =
                '<div class = "col-sm-12 panel panel-default" style="padding: 0px;">' +
                '<div class = "panel-heading">' +
                '<h2 class="panel-title fare-label" tabindex="12">' + addReviewTooltip("fare_help") + localeDictFinder['fare_filter'] + '</h2>' +
                '</div>' +
                '<div class="panel-body">' +

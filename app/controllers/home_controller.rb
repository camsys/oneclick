class HomeController < ApplicationController
  ...
  def index
    ...
    if user_signed_in?
      @actions += [
        {label: t(:plan_a_trip), target: new_user_trip_path(current_user), icon: ACTION_ICONS[:plan_a_trip]},
        {label: t(:travel_profile), target: edit_user_registration_path(current_user), icon: ACTION_ICONS[:travel_profile]},
        {label: t(:previous_trips), target: user_trips_path(current_user), icon: ACTION_ICONS[:previous_trips]},
      ]
    else
      @actions += [
        {label: t(:plan_a_trip), target: new_user_trip_path(1), icon: ACTION_ICONS[:plan_a_trip]},

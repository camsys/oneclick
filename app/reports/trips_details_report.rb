class TripsDetailsReport < AbstractReport
  ...
  def get_data(current_user, report)
    ...
    Trip.includes(:user, :creator, :trip_places, :trip_purpose, :desired_modes, :trip_parts)
      .where(trip_parts: {scheduled_time: date_option.get_date_range}).decorate
  end
  ...
  def get_columns
    cols = [:id, :created_at, :user, :assisted_by, :modes,
            ...
            :eligibility, :accommodations, :outbound_itinerary_modes,
            :return_itinerary_modes, :status, :device, :location, :trip_purpose,
